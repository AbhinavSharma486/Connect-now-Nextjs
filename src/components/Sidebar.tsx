...
const Sidebar = ({ isCollapsed, users }: SidebarProps) => {
  ...
  return (
    ...
    >
      ...
      <ScrollArea
        className='gap-2 px-2 group-[[data-collapsed=true]]:justify-center group-[[data-collapsed=true]]:px-2'
      >
        {
          users.map((user, idx) => (
            isCollapsed ? (
              ...
              <Button
                ...
                size='xl'
                className={cn(
                  "w-full justify-start gap-4 my-1", selectedUser?.email === user.email && "dark:bg-muted dark:text-white dark:hover:bg-muted dark:hover:text-white shrink"
                )}
                onClick={() => {
                  setSelectedUser(user);
                }}
